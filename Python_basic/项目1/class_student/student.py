...
# -*- coding:utf-8 -*-
import os, pymysql
import class_student.config_mysql as mysql
class Student:
    ...
    __L = []
    __count = 0
    #__db = pymysql.connect(host=mysql.host, user=mysql.user, password=mysql.password, database=mysql.database, charset=mysql.charset)
    __db = pymysql.connect(host='172.16.8.2', user='root', password='123456', database='student', charset='utf8')
    __cur = __db.cursor()
    def __repr__(self):
    ...
    def create_user(self):
        ...
        try:
            ...
            g = u.get_Human()
            self.__cur.execute("insert into info(name, age, score, status) values('%s', '%d', '%d', '%s');" % (g.get('name'), g.get('age'), g.get('score'), g.get('state')))
    ...
    #     self.__cur.execute("select user from info where name='%s';" % user)
